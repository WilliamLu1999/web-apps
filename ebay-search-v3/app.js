...
//   // let currentPrice = req.body.sellingStatus?.currentPrice?.[0]?.__value__ || 0;
//   // let currentPrice = 0;
//   //   if (req.body.sellingStatus && Array.isArray(req.body.sellingStatus)) {
//   //       const sellingStatusArray = req.body.sellingStatus;
//   //       if (sellingStatusArray.length > 0 && sellingStatusArray[0].currentPrice && Array.isArray(sellingStatusArray[0].currentPrice)) {
//   //           const currentPriceArray = sellingStatusArray[0].currentPrice;
//   //           if (currentPriceArray.length > 0 && currentPriceArray[0].__value__) {
//   //               currentPrice = parseFloat(currentPriceArray[0].__value__) || 0;
//   //           }
//   //       }
//   //   }
//   let shippingServiceCost = req.body.shippingInfo?.shippingServiceCost?.[0]?.__value__ || 0;
//   let condition = Array.isArray(req.body.condition) ? req.body.condition[0] : req.body.condition;
...
//       shippingServiceCost: parseFloat(shippingServiceCost),
...
app.post('/addItemToCart', async (req, res) => {
  console.log('Received a POST request to /addItemToCart');
  const { itemId, galleryURL, title, currentPrice, shippingServiceCost, postalCode, condition } = req.body;
  ...
  const newItem = new Item({ itemId, galleryURL, title, currentPrice, shippingServiceCost, postalCode, condition });
...
app.get('/checkItemInWishlist', async (req, res) => {
  const itemId = req.query.itemId;
  try {
      const itemExists = await Item.exists({ itemId: itemId });
      res.send(itemExists ? 'true' : 'false');
  } catch (err) {
      res.status(500).json({ success: false, message: 'Server error', error: err.message });
  }
});
...
app.get('/zipcodes', async (req, res) => {
  ...
  const APIURL = `http://api.geonames.org/postalCodeSearchJSON?postalcode_startsWith=${zipcode}&maxRows=5&username=wlu98761&country=US`;
  try {
